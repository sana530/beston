	       	...
	       	if(result.goods_all_num >0){
	            ...
	            for (i = 0; i < data.length; i++)
	            {
	            	...
	            	html+="<td class='name'><a href='"+SITEURL +"/index.php?act=goods&goods_id="+data[i]['goodsid']+"' title='"+data[i]['gname']+"' target='_top'>"+data[i]['gname']+"</a></td>";
		          	html+="<td class='price'><p>&#36;"+data[i]['price']+"×"+data[i]['num']+"</p><p><a href='javascript:void(0)' onClick='drop_topcart_item("+data[i]['storeid']+","+data[i]['specid']+");' style='color: #999;'>Delete</a></p></td>";
		          	html+="</tr>";
		        }
	         	html+="<tr><td colspan='3' class='no-border'><span class='all'>共<strong class='goods_num'>"+result.goods_all_num+"</strong>种商品   金额总计:<strong id='cart_amount'>&#36;"+result.goods_all_price+"</strong></span><span class='button' ><a href='"+SITEURL+"/index.php?act=cart' target='_top' title='结算商品' style='color: #FFF;' >结算商品</a></span></td></tr>";
	      }else{
...
(function($) {
    $.show_nc_login = function(options) {
        ...
        var login_dialog_html = $('<div class="quick-login" style="width:280px;"></div>');
        var ref_url = document.location.href;
        login_dialog_html.append('<link rel="stylesheet" type="text/css" href="'+SHOP_SITE_URL+'/templates/default/css/sregister.css"><form class="bg" method="post" id="ajax_login" action="'+APP_SITE_URL+settings.action+'"></form>').find('form')			
        	...
        	.append('<div class="enter"><input type="submit" name="Submit" value="&nbsp;" class="submit" style="float:left;"><input type="hidden" name="ref_url" value="'+ref_url+'"><span class="other"><a title="QQ" class="qq" style="float:left;margin-right:3px;display:inline" href="http://www.shopnctest.com/o2o/2.2/demo/shop/api.php?act=toqq"></a><a style="float:left;" href="http://www.shopnctest.com/o2o/2.2/demo/shop/api.php?act=tosina" title="新浪微博" class="sina">&nbsp;</a></span></div>');
