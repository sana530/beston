    ...
    public function process() {
            ...
            if ($billType['sms_notify']) {
                ...
                if ($user['mobile']) {
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_bill_order_confirm', $user['mobile'], array(
                            'billtype' => $billType['bill_type_name']
                        ));
                    } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_bill_order_confirm', $user['mobile'], array(
                            'billtype' => $billType['bill_type_name']
                        ));
                    } else {
        ...
        if($status == 2){
            ...
            if ($billType['sms_notify']) {
                ...
                if ($user['mobile']) {
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_bill_order_refund', $user['mobile'], array(
                            'billtype' => $billType['bill_type_name'],
                            'memo' => $value
                        ));
                    } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {//大于短信
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_bill_order_refund', $user['mobile'], array(
                            'billtype' => $billType['bill_type_name'],
                            'memo' => $value
                        ));
                    } else {
