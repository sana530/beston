	...
	//添加验证
	 private function createCheck() {
        $data = $this->checkFields($this->_post('data', false), $this->create_fields);
            ...
            $this->baoError('请您填写手机号码');
     	    }
			if (!isPhone($data['user_mobile']) && !isMobile($data['user_mobile']) && !is0800($data['user_mobile']) && !isCnMobile($data['user_mobile'])) {
	 ...
	 private function editCheck(){
			...
			if (!isPhone($data['user_mobile']) && !isMobile($data['user_mobile']) && !isCnMobile($data['user_mobile'])) {
            $this->baoError('联系电话格式不正确');
            }
