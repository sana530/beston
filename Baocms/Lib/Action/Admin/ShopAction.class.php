...
class ShopAction extends CommonAction{
    ...
    public function select()
    {
        ...
        $ids = array();
        foreach ($list as $k => $val) {
            $shopdetails = D('Shopdetails')->find($val['shop_id']);
            $list[$k]['details'] = $shopdetails['details'];
            $list[$k]['price'] = $shopdetails['price'];
            if ($val['user_id']) {
                $ids[$val['user_id']] = $val['user_id'];
            }
        }
    ...
    private function createCheck()
    {
        ...
        $data['user_id'] = ($data['user_id'] == '') ? NULL : (int) $data['user_id'];
    ...
    public function edit($shop_id = 0)
    {
        if ($shop_id = (int) $shop_id) {
            ...
            if ($this->isPost()) {
                ...
                $user = (empty($detail['user_id'])) ? NULL : D('Users')->find($detail['user_id']);
                $this->assign('areas', D('Area')->fetchAll());
                $this->assign('cates', D('Shopcate')->fetchAll());
                $this->assign('business', D('Business')->fetchAll());
    ...
    private function editCheck($shop_id)
    {
        ...
        if (!isImage($data['logo']) && !(filter_var($data['photo'], FILTER_VALIDATE_URL))) {
            $this->baoError('商铺LOGO格式不正确');
        }
        $data['photo'] = htmlspecialchars($data['photo']);
        if (empty($data['photo'])) {
        ...
        if (!isImage($data['photo']) && !(filter_var($data['photo'], FILTER_VALIDATE_URL))) {
            $this->baoError('店铺缩略图格式不正确');
        }
        $data['addr'] = htmlspecialchars($data['addr']);
        if (empty($data['addr'])) {
