...
class CommonAction extends Action{
    ...
    protected function _initialize(){
        ...
        $admin_allow_user = explode('|', $this->_CONFIG['site']['admin_allow_user']);//p($admin_allow_user);die;
        $user_id = getUid();
        if (!in_array($user_id, $admin_allow_user) || $user_id == 0) {
            header('Location: ' . __HOST__ . '/mobile/error.html');die;
        }
