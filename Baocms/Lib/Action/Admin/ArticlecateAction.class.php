...
class ArticlecateAction extends CommonAction {
    ...
    private $create_fields = array('cate_name', 'orderby','cate_type');
    private $edit_fields = array('cate_name', 'orderby','cate_type');


    public function index() {
        $Articlecate = D('Articlecate');
        $list = $Articlecate->fetchAll();//所有主分类按照所属版块重新排列
        $this->assign('list', $list); // 赋值数据集
        $catetype = array('普通文章','会员手册','商家手册','管理员手册');//显示分类的所属版块名称
        $this->assign('catetype',$catetype);//赋值版块名称
    ...
    private function createCheck() {
        ...
        $data['cate_type'] = htmlspecialchars($data['cate_type']);
        if (!is_numeric($data['cate_type'])) {
            $this->baoError('所属版块不能为空');
        }
