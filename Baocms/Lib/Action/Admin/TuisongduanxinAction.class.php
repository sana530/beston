    ...
    public function delete($tuisong_id = 0) {
	 ...
	 public function tuisong($tuisong_id){
		...
		foreach($users as $value){
            if (isCnMobile($value['mobile'])) {
                D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_yzm', $value['mobile'], array(
                    'sitename' => $this->_CONFIG['site']['sitename'],
                    'code' => $detail['title'],//短信内容
                ));
            } elseif (isMobile($value['mobile'])) {
                if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                    D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_yzm', $value['mobile'], array(
                        'sitename' => $this->_CONFIG['site']['sitename'],
                        'code' => $detail['title'],//短信内容
                    ));
