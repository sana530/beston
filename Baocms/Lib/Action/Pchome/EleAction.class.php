...
class EleAction extends CommonAction
{
    ...
    public function order()
    {
        ...
        $is_delivery = isset($_POST['delivery'])? (($this->_post('delivery', 'intval') == 1)? 1 : 0) : 0;
        $shop_id = 0;
        $shops = array();
        $products = array();
        ...
        //区分外卖中的送餐与自取，如果商家是不送餐商家，则不允许进入送餐页面
        if (($shop['is_delivery'] == 0) && ($is_delivery == 1)) {
            $this->ajaxReturn(array('status' => 'error', 'msg' => '商家未开通送餐服务'));
        }
		
		$settlement_price = (int) ($total['money'] - ($total['money']* $shop['rate'] / 1000));//先算出结算价，不按照后台为准
        $_shop = D('Shop')->find($shop_id);
        //如果是自取的话，不计算运费
        ($is_delivery == 1 && $_shop['is_pei'] == 1) && ($total['money'] += $shop['logistics']);
        $logistics = ($is_delivery == 1 && $_shop['is_pei'] == 1) ? $shop['logistics'] : 0;
        $total['need_pay'] = $total['money'];
        //后面要用到计算。只有外送的单才会检查是否有起送价
        if (($shop['since_money'] > $total['money']) && $is_delivery == 1 && $_shop['is_pei'] == 1) {
        ...
        if ($order_id = D('Eleorder')->add(array(
            ...
            $this->ajaxReturn(array('status' => 'success',
                'msg' => ($is_delivery == 1) ? '下单成功！您可以选择配送地址!' : '下单成功！付款后请到店用餐或自行取餐！',
                'url' => U('ele/pay', array('order_id' => $order_id))));
        }
    ...
    public function pay()
    {
        ...
        $this->assign('city', D('city')->find($addrs['city_id']));
        $this->assign('area', D('area')->find($addrs['area_id']));
        $this->assign('business', D('business')->find($addrs['business_id']));
        $this->display();
    }
    ...
    public function pay2()
    {
        ...
        //只有需要送餐服务时，才查看是否有传递地址信息
        if (empty($addr_id) && $order['is_delivery']==1) {
    ...
    public function shop(){
        ...
        //区分外卖中的送餐与自取，如果商家是不送餐商家，则不允许进入送餐页面
        $is_delivery = isset($_GET['delivery'])? (($this->_get('delivery', 'intval') == 1)? 1 : 0) : 0;
        if (($detail['is_delivery'] == 0) && ($is_delivery == 1)) {
            $this->error('商家未开通送餐服务');
        }
        ...
        $this->assign('is_delivery', $is_delivery);
    ...
    public function index()
    {
        ...
        foreach ($lists as $k => $val) {
            $lists[$k]['d'] = getDistanceCN($lat, $lng, $val['lat'], $val['lng']);
        }
    ...
    public function distance()
    {
        ...
        $shop = D('Shop')->find($ele_order['shop_id']);
        $addr = htmlspecialchars($_POST['addr']);
        $this->assign('order_id',$order_id);
        $this->assign('from_addr_lat', $shop['lat']);
        $this->assign('from_addr_lng', $shop['lng']);
        $this->assign('to_addr', $addr);
        $this->display();
    }
    ...
    {
        $order_id = (int) $_POST['order_id'];
        $distance = preg_replace('/[^\d.]/','',htmlspecialchars($_POST['distance']));
        $config = D('Setting')->fetchAll();
        $delivery['basickm'] = $config['site']['deliverybasickm'];
        $delivery['basicfee'] = $config['site']['deliverybasicfee'];
        $delivery['perkm'] = $config['site']['deliveryperkm'];
        $delivery['perfee'] = $config['site']['deliveryperfee'];
        if ($distance > $config['site']['deliverybasickm']) {
            $logistics = ($config['site']['deliverybasicfee'] + ($distance - $config['site']['deliverybasickm']) / $config['site']['deliveryperkm'] * $config['site']['deliveryperfee']) * 100;
        } else {
            $logistics = ($config['site']['deliverybasicfee']) * 100;
        }
        $order = D('Eleorder')->find($order_id);
        $total_price = $order['total_price'] - $order['logistics'] + $logistics;
        D('Eleorder')->save( array('order_id' => $order_id, 'logistics' => $logistics, 'total_price' => $total_price, 'need_pay' => $total_price ) );
        $this->ajaxReturn(array('status' => 'success', 'total_price' => $total_price, 'logistics' => $logistics));
    }
}