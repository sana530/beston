...
class AppointAction extends CommonAction {
    ...
    public function create($appoint_id) {
		...
		if ($this->member['money'] < $cate_id['price']){
			$this->baoSuccess('抱歉，您的余额不足',U('member/money/money'));
		}
        ...
        if ($data['data']['country']=='cn') {
            if (!isCnMobile($data['tel'])) {
                $this->baoError('请输入正确的中国手机号码');
            }
        }
        if ($data['data']['country']=='nz') {
            if (!isMobile($data['tel']) && !isPhone($data['tel'])) {
                $this->baoError('请输入正确的NZ手机号码');
            }
        }
        ...
        if ($order_id = D('Appointorder')->add($data)) {
            ...
            if (isCnMobile($user_mobile)) {
                D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_appoint_TZ_user', $user_mobile, array(
                ...
                if($this->_CONFIG['sms']['dxapi'] == 'bu') {
                    D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_appoint_TZ_user', $user_mobile, array(
                        'sitename'=>$this->_CONFIG['site']['sitename'],
                        'name' => $data['name'],
                        'time' => $sms_time,
                        'addr' => $data['addr'],
