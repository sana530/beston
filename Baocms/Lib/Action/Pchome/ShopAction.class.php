...
class ShopAction extends CommonAction
{
    ...
    public function index()
    {
        ...
        foreach ($list as $k => $val) {
            $list[$k]['d'] = getDistanceCN($lat, $lng, $val['lat'], $val['lng']);
    ...
    public function apply()
    {
        ...
        if ($this->isPost()) {
            ...
            if (!is_numeric($data['lng']) || !is_numeric($data['lat'])) {
                $this->baoError('建议您使用我们的工具,选择地址!', 2000, true);
            }
    ...
    public function tui()
    {
        ...
        if ($this->isPost()) {
            ...
            if (!isMobile($account['account']) && !isEmail($account['account']) && !isCnMobile($account['account'])) {
                session('verify', null);
    ...
    private function tuiCheck()
    {
        ...
        if (!isPhone($data['tel']) && !isMobile($data['tel']) && !isMobile($data['tel'])) {
    ...
    public function yuyue2()
    {
        ...
        if (IS_AJAX) {
            $data = $this->checkFields($this->_post('data', false), array('name', 'mobile', 'country', 'content', 'yuyue_date', 'yuyue_time', 'number'));
            ...
            if ($data['country'] == 'nz') {
                if (!isMobile($data['mobile'])) {
                    $this->ajaxReturn(array('status' => 'error', 'msg' => '请输入正确的NZ手机号码'));
                }
            } elseif ($data['country'] == 'cn') {
                if (!isCnMobile($data['mobile'])) {
                    $this->ajaxReturn(array('status' => 'error', 'msg' => '请输入正确的中国手机号码'));
                }
            }
            ...
            if ($obj->add($data)) {
                //通知用户
                if (isCnMobile($data['mobile'])) {
                    D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_shop_yuyue_code', $data['mobile'], array('shop_name' => $detail['shop_name'], 'shop_tel' => $detail['mobile'], 'shop_addr' => $detail['addr'], 'code' => $data['code']));
                } elseif (isMobile($data['mobile'])) {
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_shop_yuyue_code', $data['mobile'], array('shop_name' => $detail['shop_name'], 'shop_tel' => $detail['mobile'], 'shop_addr' => $detail['addr'], 'code' => $data['code']));
                ...
                if (isCnMobile($detail['mobile'])) {
                    ...
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        ...
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_shangjia_yuyue', $detail['mobile'], array('name' => $data['name'], 'content' => $data['content'], 'yuyue_date' => $data['yuyue_date'], 'mobile' => $data['mobile'], 'number' => $data['number']));
                    } else {
    ...
    public function ping()
    {
        ...
        // 赋值分页输出
        $this->assign('pic', $pic = D('Shoppic')->where(array('shop_id' => $shop_id))->order(array('pic_id' => 'desc'))->count());
        $this->assign('ex', $ex = D('Shopdetails')->changeBTime(D('Shopdetails')->find($shop_id)));
        $this->assign('detail', $detail);
        $this->display();
    }
    //团
