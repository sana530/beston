...
class TuanAction extends CommonAction {
    ...
    public function index() {
        ...
        if ($free = (int) $this->_param('free')) {
            $linkArr['free'] = $free;
            $map['tuan_price'] = array('EQ', '0');
        }
    ...
    public function pay2() {
        ...
        if ($code == 'wait') { // 到店付 将不会有支付记录，并且不能使用在线的积分
            ...
            if (D('Tuanorder')->save(array('order_id' => $order_id, 'status' => '-1'))) { //更新成到店付的状态
                ...
                $codestr = join(',', $codes);
                if (isCnMobile($this->member['mobile'])) {
                    D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_tgdx', $this->member['mobile'], array(
                        'sitename'=>$this->_CONFIG['site']['sitename'],
                        'code' => $codestr,
                        'nickname' => $this->member['nickname'],
                        'tuan' => $tuan['title']
                    ));
                } elseif (isMobile($this->member['mobile'])) {
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_tgdx', $this->member['mobile'], array(
                            'sitename'=>$this->_CONFIG['site']['sitename'],
                            'code' => $codestr,
                            'nickname' => $this->member['nickname'],
                            'tuan' => $tuan['title']
