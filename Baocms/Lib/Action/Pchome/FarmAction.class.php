...
class FarmAction extends CommonAction {
    ...
    public function order(){
        ...
        if(!$gotime){
            ...
            $this->error('没有填写联系人！');
        }else if(!$mobile || !isMobile($mobile) || !isCnMobile($mobile)){
    ...
    public function orderCreate(){
        ...
        if (empty($this->uid)) {
            ...
            if(!$data['gotime']){
                ...
                $this->error('没有填写联系人！');
            }else if(!$data['mobile'] || !isMobile($data['mobile']) || !isCnMobile($data['mobile'])){
                $this->error('手机号码不正确！'.$data['mobile']);
            }else if(!$data['pid']){
                $this->error('没有选择套餐！');
            }else{
    ...
    public function pay2(){
        ...
        D('Weixintmpl')->weixin_notice_farm_user($order_id,$this->uid,1);//旅行社微信通知用户   
