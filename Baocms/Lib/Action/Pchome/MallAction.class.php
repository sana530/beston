...
class MallAction extends CommonAction
{
   ...
   function goods(){
    ...
    public function order(){
        ...
        $totall_express = $needpay = array();
        $expressfee = $totalexpressfee =0;
        foreach ($goods as $val) {
            ...
            $needpay[$val['shop_id']] += $need_pay;
            $totall_express[$val['shop_id']] = D('Weidiandetails')->where(array('shop_id' => $val['shop_id']))->getfield('express_price');
        ...
        //订单驿站站长记录
        $tui_zhanzhang = D('Users')->where(array('user_id'=>$this->uid))->getField('tui_zhanzhang');
        if ($tui_zhanzhang > 0) {
            $order['tui_zhanzhang'] = $tui_zhanzhang;
        }
        ...
        foreach ($ordergoods as $k => $val) {
            ...
            $order['total_express'] = $totall_express[$k];
            $order['need_pay'] = $needpay[$k]+$totall_express[$k]*100;
            //这里影响的没有减去积分
            ...
            if(!D('Order')->autoCheckToken($_POST)){
                $this->baoSuccess('亲！，订单已存在！', U('member/order/goods'));
            }
            if ($order_id = D('Order')->add($order)) {
                // 更新needpay 减去优惠券抵扣金额
                $Coupon = D('Coupon')->Obtain_Coupon($order_id,$this->uid);
                $newneedpay = $order['need_pay']-$Coupon['reduce_price'];
                $data = array(
                    'need_pay' => $newneedpay,
                    'download_id' => $Coupon['download_id'],
        ...
        if (count($order_ids) > 1) {
            ...
            $need_pay = $need_pay + $orderexpress['total_express']*100;
    ...
    public function paycode()
    {
        ...
        //运费，可用积分抵扣金额
        $total_express = $use_integral =0; 
        $order=D('Order')->where(array('order_id' => array('IN', $order_ids)))->select();
        foreach ($order as $k => $val) {
            $total_express += $val['total_express'];
        ...
        $detail['total_express']=$total_express;
    ...
    public function pay2(){
        ...
        if ($code == 'wait') {
            ...
            $need_pay += $order['total_express']*100; //保留原来逻辑，直接+订单的物流配送费用 
            if (empty($logs)) {
                $logs = array(
					'type' => 'goods', 
					'user_id' => $this->uid, 
