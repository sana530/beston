...
class PassportAction extends CommonAction
{
    ...
    {
        if ($this->isPost()) {
            if (isMobile(htmlspecialchars($_POST['data']['account'])) || isCnMobile(htmlspecialchars($_POST['data']['account']))) {
    ...
    private function createCheck()
    {
        ...
        if (!isMobile($data['account']) && !isEmail($data['account']) && !isCnMobile($data['account'])) {
            session('verify', null);
    ...
    public function findsms()
    {
        ...
        if ($user = D('Users')->getUserByAccount($account)) {
            if (empty($user['mobile'])) {
                ...
                if ($user['mobile'] != $mobile) { die('手机号不正确，请填写您的绑定手机号！'); }
            }
        }*/
        $randstring = session('scode');
        if (empty($randstring)) {
    ...
    public function newpwd()
    {
        ...
        if ($way == 1) {
            ...
            if (empty($email)) {
                $this->baoError('请输入邮箱地址');
            }
            $user = D('Users')->getUserByEmail($email);
            if ($email != $user['email']) {
                $this->baoError('请输入正确的认证邮件');
            }
            ...
            D('Passport')->uppwd($user['account'], '', $password);
            if (isCnMobile($mobile)) {
                D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_user_newpwd', $mobile, array(
                    'sitename' => $this->_CONFIG['site']['sitename'],
                    'newpwd' => $password
                ));
            } elseif (isMobile($mobile)) {
                if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                    D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_user_newpwd', $mobile, array(
                        'sitename' => $this->_CONFIG['site']['sitename'],
                        'newpwd' => $password
                    ));
    ...
    public function cbscan () {
        ...
        //成功登陆后，跳转到用户中心页
        setUid($user_id);
        redirect(U('member/index/index'),0);
    }
