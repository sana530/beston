...
class SetAction extends CommonAction{
    ...
    public function sendsms()
    {
        ...
        if ($this->_post('country') == 'nz') {
            if (!isMobile($mobile)) {
                $this->ajaxReturn(array('status' => 'error', 'msg' => '请输入正确的NZ手机号码'));
            }
        } elseif ($this->_post('country') == 'cn') {
    ...
    public function paypass() {
        ...
        if ($this->isPost()) {
            ...
            if (D('Users')->data($save)->where(array('user_id' => $uid))->save()) {
                $this->baoSuccess('支付密码信息已修改', U('member/index/index'));
            } else {
                $this->baoError('修改失败，请重新修改');
            }
        } else {
