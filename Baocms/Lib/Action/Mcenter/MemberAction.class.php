...
class MemberAction extends CommonAction{
    public function pay(){
        ...
        if ($member['money'] + $member['money_negative'] < $detail['need_pay']) {
        ...
        } elseif($detail['type'] == 'api'){
            $this->api_success('恭喜您支付成功', $detail);die;
        } elseif($detail['type'] == 'receipt'){
            $this->receipt_success('恭喜您支付成功', $detail);die;
        } else {
    ...
    protected function ele_success($message, $detail){
        ...
        $detail['is_delivery'] = $eleorder['is_delivery'];
    ...
    protected function goods_success($message, $detail){
        ...
        $order =D('Order')->field('sum(total_express) as total_express,sum(use_integral) as use_integral,sum(coupon_price) as coupon_price ')->where(array('order_id' => array('IN', $order_ids)))->find();
        $detail['express_price']=$order['total_express'];
        $detail['use_integral'] =$order['use_integral'];
        $detail['coupon_price']=$order['coupon_price'];
        
        if (count($order_ids)>1){
            $detail['MultipleOrder']=1; // 多订单问题
        }else{
            $detail['MultipleOrder']=0; 
        }
    ...
    protected function api_success($message, $detail) {
        $scanOrder = D('Scanorder') -> find($detail['order_id']);
        S('scan_paid_result_'.$scanOrder['token'],$scanOrder['token'],300); //写缓存文件，作为被扫描设备异步轮询查询条件
        $this -> assign('order', $scanOrder);
        $this -> assign('message', $message);
        $this -> assign('detail', $detail);
        $this -> assign('paytype', D('Payment') -> getPayments());
        $this -> display('common');
    }
    protected function receipt_success($message, $detail) {
        $receiptOrder = D('Shopreceiptorder') -> find($detail['order_id']);
        $this -> assign('order', $receiptOrder);
        $this -> assign('message', $message);
        $this -> assign('detail', $detail);
        $this -> assign('paytype', D('Payment') -> getPayments());
        $this -> display('common');
    ...
    public function mobile()
    {
        ...
        if ($this->isPost()) {
            ...
            $mobile = str_replace(array("+", " ", "(", ")", "-",), "", trim($mobile));
            if (isMobile($mobile)) {
                (preg_match('/^64[0-9]{6,9}$/', $mobile)) && $mobile = '0'.substr($mobile, 2);
            }
    ...
    public function sendsms()
    {
        ...
        if (isCnMobile($mobile)) {
            ...
            } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {//如果开启大于
