...
class ExpressAction extends CommonAction{
    ...
    public function create( ){
        $money = $this->_CONFIG['mobile']['delivery_price']*100;
        if ( $this->isPost( ) ){
            ...
            if ( $express_id = d( "Express" )->add( $data ) ){
                if(D('Users')->addMoney($this->uid, -$money,'众包快递')){
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_delivery_notice_customer', $this->_CONFIG['site']['config_mobile'], $data);
                    } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {//大于短信
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_delivery_notice_customer', $this->_CONFIG['site']['config_mobile'], array('sitename' => $this->_CONFIG['site']['sitename'], 'code' => $data));
                    } else {
                        D('Sms')->sendSms('sms_delivery_notice_customer', $this->_CONFIG['site']['config_mobile'], array('code' => $data));
                    }
                    $this->fengmiMsg( "您已支付并发布成功，请等待配送员联系。", u( "express/index" ) );
                }
            }
            $this->fengmiMsg( "发布失败" );
        }
        else{
            $this->assign('delivery_price', $money/100);
    ...
    public function createCheck( ){
        ...
        if ( !isMobile( $data['from_mobile'] ) && !isCnMobile( $data['from_mobile'] ) ){
        ...
        if ( !ismobile( $data['to_mobile'] ) && !isCnMobile( $data['to_mobile'] ) ){
