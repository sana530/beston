...
public function booking($order_id) {
	...
	public function respond() {
		...
		if (!empty($detail)) {
			if ($detail['type'] == 'ele') {
                ...
                $this -> api_success('恭喜您支付成功啦！', $detail);
            } elseif ($detail['type'] == 'receipt') {
                $this -> receipt_success('恭喜您支付成功啦！', $detail);
            } else {
	...
	public function yes($log_id) {
		...
		switch ($logs['type']) {
			...
			case 'pintuan' :
				...
				break;
            case 'api' :
                $this -> api_success('恭喜您支付成功啦！', $logs);
                break;
            case 'receipt' :
                $this -> receipt_success('恭喜您支付成功啦！', $logs);
                break;
			default :
				$this -> success('恭喜您充值成功', U('mcenter/index/index'));
				break;
	...
	public function payment($log_id) {
		...
		// 商城购物 才有积分抵现  // 使用积分（单店)
		if  ($logs['type'] == 'goods') {
			if ($logs['order_id']>0 && empty($logs['order_ids']) ) {
				$use_integral =D('Order')->where(array('order_id' => $logs['order_id']))->getfield('use_integral');
			}
		}

		//如果是商家收款，则最终确认一遍红包的合法性
        if ($logs['type'] == 'receipt') {

		    $receiptOrder = D('Shopreceiptorder');
		    $order = $receiptOrder->where(array('order_id'=>$logs['order_id']))->find();

		    //检查订单中是否有红包记录
		    if ($order['receive_id']) {

                $where = array('receive_id'=>$order['receive_id'], 'is_used'=>0);
                ...
                //如果红包不合法，更新两个表中need_pay,并清除红包相关信息
                if ($valid == false) {
                    $data = array('receive_id'=>NULL, 'hongbao_price'=>NULL, 'need_pay'=>$order['total_price']);
                    $receiptOrder->data($data)->where(array('order_id'=>$logs['order_id']))->save();
                    D('Paymentlogs')->data(array('need_pay'=>$order['total_price']))->where(array('log_id'=>$logs['log_id']))->save();
                    $this->error('您的红包已失效，价格已被修改', U('payment/payment', array('log_id'=>$log_id)));
                }
		...
		$logs['use_integral'] =$use_integral;
