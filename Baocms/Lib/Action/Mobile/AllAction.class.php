...
class AllAction extends CommonAction {
    ...
    public function load(){
		...
		$list_activity=$Activity->Field('"活动" as t_name,"huodong/detail" as t_url,"activity_id" as t_param,activity_id as t_id,title as t_title,photo as t_photo,FROM_UNIXTIME(create_time,"%Y-%m-%d") as t_note')->order($orderby)->where($where_activity)->select();
		...
		$list_farm=$Farm->Field('"旅行社" as t_name,"farm/detail" as t_url,"farm_id" as t_param,farm_id as t_id,farm_name as t_title,photo as t_photo,FROM_UNIXTIME(create_time,"%Y-%m-%d") as t_note')->order($orderby)->where($where_farm)->select();
		...
		$list_hotel=$Hotel->Field('"酒店" as t_name,"hotel/detail" as t_url,"hotel_id" as t_param,hotel_id as t_id,hotel_name as t_title,photo as t_photo,concat("$",round(price/100,2)) as t_note')->order($orderby)->where($where_hotel)->select();
