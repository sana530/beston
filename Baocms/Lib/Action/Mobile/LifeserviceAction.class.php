...
class LifeserviceAction extends CommonAction
{
    ...
    public function create($id) {
       ...
       if (!$id = (int) $id) {
        ...
        if (!($data['mobile'] = $this->_post('mobile', 'htmlspecialchars'))) {
            $this->fengmiMsg('联系电话不能为空');
        }
        $data['data'] = $this->_post('data', 'htmlspecialchars');
        if ($data['data']['country'] == 'cn') {
            if (!isCnMobile($data['mobile'])) {
                $this->fengmiMsg('请输入正确的中国手机号码');
            }
        }
        if ($data['data']['country'] == 'nz') {
        ...
        if (D('Housework')->add($data)) {
            ...
            if (isCnMobile($user_mobile)) {
                D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_lifeservice_TZ_user', $user_mobile, array(
            ...
            } elseif (isMobile($user_mobile)) {
                if($this->_CONFIG['sms']['dxapi'] == 'bu') {
                    D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_lifeservice_TZ_user', $user_mobile, array(
                        'sitename'=>$this->_CONFIG['site']['sitename'],
                        'name' => $data['name'],
                        'time' => $sms_time,
                        'addr' => $data['addr'],
