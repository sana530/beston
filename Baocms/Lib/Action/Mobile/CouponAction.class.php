...
class CouponAction extends CommonAction {
    ...
    public function index() {
        ...
        $coupon = fetchCalldata(array('mdl'=>'Coupon', 'where'=>'audit=1 AND closed=0 AND expire_date >= "'.TODAY.'" AND city_id = '.$this->city_id,'cache'=>21600));
    ...
    public function download() {
        ...
        if (D('Coupondownload')->add($data)) {
            ...
            if (isCnMobile($this->member['mobile'])) {
                D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_dytz', $this->member['mobile'], array(
                ...
                if($this->_CONFIG['sms']['dxapi'] == 'bu') {
                    D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_dytz', $this->member['mobile'], array(
                        'sitename'=>$this->_CONFIG['site']['sitename'],
                        'coupon_title' => $detail['title'],
                        'shop_name' => $shop['shop_name'],
                        'code' => $code,
