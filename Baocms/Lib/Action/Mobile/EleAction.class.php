...
class EleAction extends CommonAction
{
    ...
    public function cart(){
        ...
        $is_delivery = isset($_GET['delivery'])? (($this->_get('delivery', 'intval') == 1)? 1 : 0) : 0; //区分外卖中的送餐与自取
        if ($goods = cookie('ele')) {
    ...
    public function order(){
        ...
        if ($order_id = D('Eleorder')->add(array(
            ...
            cookie('ele', null);
            $this->fengmiMsg(($is_delivery == 1) ? '下单成功！您可以选择配送地址!' : '下单成功！付款后请到店用餐或自行取餐！', U('ele/pay', array('order_id' => $order_id)));
        }
