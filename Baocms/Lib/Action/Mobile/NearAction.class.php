...
class NearAction extends CommonAction{
	...
	//首页城市定位  可定位至4级区域，区/县
		...
		$city = mb_substr($addr['city'], 0, null, 'utf-8');
    ...
    public function loaddata(){
        ...
        $key = str_replace(' ', '+', $key);
        $arr = D('Cloud')->NearData($lat, $lng, $key, $type);
        $num = intval($arr->status);
        $result = $arr->results;

        if ($num == 0) {
            foreach ($result as $value) {
                $value->uid = $value->place_id;
                $value->location->lat = $value->geometry->location->lat;
                $value->location->lng = $value->geometry->location->lng;
                $value->address = $value->vicinity;
                ...
                $data['score'] = ($value->rating) *10;
                $data['tag'] = $value->types;
                $data['orderby'] = 1000;
                ...
                if ($collects_open == 1) {
                    if ($citys_name['pinyin'] == $city_code) {
                        if ($type == $collects_community) {

                            $Community = D('Community')->where(array('uid' => $value->uid))->find();
                            if (!empty($Community['community_id'])) {
                                $data['telephone'] = $Community['tel'];
                            } else {
                                $data['telephone'] = D('Cloud')->GetDetail($value->place_id)->result->formatted_phone_number;
                                if (!empty($data['telephone'])) {
                                    $communitys['tel'] = $data['telephone'];
                                } else {
                            ...
                            if (!empty($shop['shop_id'])) {
                                $data['telephone'] = $shop['tel'];
                                $data['score'] = $shop['score'];
                            } else {
                                $data['telephone'] = D('Cloud')->GetTel($value->place_id, $this->_CONFIG['site']['tel']);
                                $shops['tel'] = $data['telephone'];
                                D('Shop')->add($shops);
                            }
    ...
    public function detail($id){
        ...
        if (empty($detail)) {
            //检查是否是商家
            $detail = D('Shop')->where(array('uid' => $id))->find();
            ($detail['addr']) && $detail['address'] = $detail['addr'];
            ($detail['tel']) && $detail['telephone'] = $detail['tel'];
            ($detail['name']) && $detail['shop_name'] = $detail['name'];
        } else {
            //本地没有数据到远程获取
            if(!is_numeric($id)){
                $googleurl = "https://maps.googleapis.com/maps/api/place/details/json?placeid=$id&key=AIzaSyBSoyUbiTvaa1kbqgVQj43kv46SsaKvjSU";
                $googlearr = file_get_contents_url($googleurl);
                $detail['uid'] = $googlearr->result->place_id;
                $detail['name'] = $googlearr->result->name;
                $detail['types'] = $googlearr->result->types[0];
                $detail['lat'] = $googlearr->result->geometry->location->lat;
                $detail['lng'] = $googlearr->result->geometry->location->lng;
                $detail['telephone'] = $googlearr->result->international_phone_number;
                $detail['address'] = $googlearr->result->formatted_address;
                $detail['tag'] = $googlearr->result->types[1];
            }
        }
