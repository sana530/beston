...
class AddrAction extends CommonAction{
    public function _initialize(){
        parent::_initialize();
        if ($this->_CONFIG['operation']['addr'] == 0) {
            $this->error('此功能已关闭');
            die;
        }
    }
    ...
    public function add_addr(){
        if (empty($this->uid)) {
            ...
            $city_name = I('city_name', '', 'trim,htmlspecialchars');
            $area_name = I('area_name', '', 'trim,htmlspecialchars');
            $business_name = I('business_name', '', 'trim,htmlspecialchars');
            $mobile = I('mobile', '', 'trim');
            $addr = I('addr', '', 'trim,htmlspecialchars');
            ($city_name) && $addr = explode(",", $addr)[0];
            if (!$name) {
