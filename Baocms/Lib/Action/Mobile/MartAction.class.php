...
class MartAction extends CommonAction{
    public function _initialize(){
	    ...
	    foreach ($weidiancates as $key => $v) {
            ...
            $map['audit'] = 1;
            $map['closed'] = 0;
            $count = D('Weidiandetails')->where($map)->count(); // 统计当前分类记录
    ...
    public function index(){
        ...
        $this->assign("cat", $cat);
        $parent_id = D('Weidiancate')->getParentsId($cat);
        if ($parent_id == 0) {
            $top_cat = $cat;
        } else {
            $top_cat = $parent_id;
    ...
    public function cart(){
        ...
        if (empty($backurl)) {
            $backurl = U('mart/lists',array('id'=>$id));
        }
        $goods = $this->_cart;
        if ( empty( $goods ) ){
            $this->error( "亲还没有选购产品呢!", U	( "mart/lists", array("id" => $id)));
        }
        $shop_detail = D('Shop')->where(array('shop_id'=>$detail['shop_id']))->find();
        $is_direct_sale = $shop_detail['is_direct_sale'];
        $this->assign('collection_date', collection_date());
        $this->assign('is_direct_sale', $is_direct_sale);
