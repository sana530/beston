...
class LifeAction extends CommonAction{
    ...
    private $create_fields = array('title', 'cate_id', 'area_name', 'business_name', 'text1', 'text2', 'text3', 'num1', 'num2', 'select1', 'select2', 'select3', 'select4', 'select5', 'photo', 'contact', 'mobile', 'qq', 'addr', 'lng', 'lat');
    ...
    public function lifelist() {
        ...
        $lifes = $Life->order(" (ABS(lng - '{$lng}') +  ABS(lat - '{$lat}') ) asc ")->where($map)->limit($Page->firstRow . ',' . $Page->listRows)->select();
        foreach ($lifes as $k => $val) {
            $lifes[$k]['d'] = getDistance($lat, $lng, $val['lat'], $val['lng']);
        }
                                ...
                                if ($v['sms'] == 1) {
                                    ...
                                    if ($mobile) {
                                        if (isCnMobile($mobile)) {
                                            D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_life_dingyue_tongzhi_user', $mobile, array(
                                            ...
                                            if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                                                D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_life_dingyue_tongzhi_user', $mobile, array(
                                                    'sitename' => $this->_CONFIG['site']['sitename'],
                                                    'title' => $data['title'],
                                                    'user_name' => $data['contact'],
                                                    'user_mobile' => $data['mobile']
                                                ));
                                            } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {//大于发送分类信息短信
    ...
    private function createCheck(){
        ...
        if (!isMobile($data['mobile']) && !isPhone($data['mobile']) && !isCnMobile($data['mobile'])) {
