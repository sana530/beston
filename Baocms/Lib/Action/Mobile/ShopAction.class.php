...
class ShopAction extends CommonAction{
    ...
    public function qrcode($shop_id){
        ...
        $qrimg = baoQrCode('mobile_shop'.$shop_id, U('shop/detail', array('shop_id'=>$shop_id)));
        $img = "<img src='http://".$_SERVER['HTTP_HOST']."/attachs/".$qrimg."' style='width:100%;'>";
    ...
    public function loaddata(){
        ...
        foreach ($list as $k => $val) {
            ...
            $tuan = D('Tuan')->field('tuan_price,title')->where($map)->order(array('end_date' => 'asc'))->limit(1)->find();
            if ($tuan) {
                $list[$k]['tuan'] = $tuan;
            }
        }
    ...
    public function detail(){
        ...
        $this->assign('ex', D('Shopdetails')->changeBTime(D('Shopdetails')->find($shop_id)));
    ...
    public function book($shop_id){
        ...
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                //通知用户
                if (isCnMobile($data['mobile'])) {
                    D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_shop_yuyue_code', $data['mobile'], array('sitename' => $this->_CONFIG['site']['sitename'], 'shop_name' => $detail['shop_name'], 'shop_tel' => $detail['tel'], 'shop_addr' => $detail['addr'], 'code' => $data['code']));
                } elseif (isMobile($data['mobile'])) {
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_shop_yuyue_code', $data['mobile'], array('sitename' => $this->_CONFIG['site']['sitename'], 'shop_name' => $detail['shop_name'], 'shop_tel' => $detail['tel'], 'shop_addr' => $detail['addr'], 'code' => $data['code']));
                ...
                if ($detail['mobile']) {
                    //预约通知商家功能开始
                    if (isCnMobile($detail['mobile'])) {
                        D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_shangjia_yuyue', $detail['mobile'], array('sitename' => $this->_CONFIG['site']['sitename'], 'name' => $data['name'], 'content' => $data['content'], 'yuyue_time' => $data['yuyue_time'], 'mobile' => $data['mobile'], 'number' => $data['number'], 'yuyue_date' => $data['yuyue_date']));
                    } elseif (isMobile($detail['mobile'])) {
                        if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                            D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_shangjia_yuyue', $detail['mobile'], array('sitename' => $this->_CONFIG['site']['sitename'], 'name' => $data['name'], 'content' => $data['content'], 'yuyue_time' => $data['yuyue_time'], 'mobile' => $data['mobile'], 'number' => $data['number'], 'yuyue_date' => $data['yuyue_date']));
                        } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {
                            D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_shangjia_yuyue', $detail['mobile'], array('sitename' => $this->_CONFIG['site']['sitename'], 'name' => $data['name'], 'content' => $data['content'], 'yuyue_time' => $data['yuyue_time'], 'mobile' => $data['mobile'], 'number' => $data['number'], 'yuyue_date' => $data['yuyue_date']));
                        } else {
                            D('Sms')->sendSms('sms_shangjia_yuyue', $detail['mobile'], array('name' => $data['name'], 'content' => $data['content'], 'yuyue_time' => $data['yuyue_time'], 'mobile' => $data['mobile'], 'number' => $data['number'], 'yuyue_date' => $data['yuyue_date']));
                        }
                    }
                    //预约通知商家功能结束
                } else {
                    //如果商家没留手机时，通知网站管理员
                    $config_mobile = $this->_CONFIG['site']['config_mobile'];
                    if (isCnMobile($config_mobile)) {
                        D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_admin_yuyue', $config_mobile, array('shop_name' => $detail['shop_name'], 'name' => $data['name'], 'content' => $data['content'], 'yuyue_time' => $data['yuyue_time'], 'mobile' => $data['mobile'], 'number' => $data['number'], 'yuyue_date' => $data['yuyue_date']));
                    } elseif (isMobile($config_mobile)) {
                        if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                            D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_admin_yuyue', $config_mobile, array('shop_name' => $detail['shop_name'], 'name' => $data['name'], 'content' => $data['content'], 'yuyue_time' => $data['yuyue_time'], 'mobile' => $data['mobile'], 'number' => $data['number'], 'yuyue_date' => $data['yuyue_date']));
                        } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {
                            D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_admin_yuyue', $config_mobile, array('shop_name' => $detail['shop_name'], 'name' => $data['name'], 'content' => $data['content'], 'yuyue_time' => $data['yuyue_time'], 'mobile' => $data['mobile'], 'number' => $data['number'], 'yuyue_date' => $data['yuyue_date']));
                        } else {
                            D('Sms')->sendSms('sms_admin_yuyue', $config_mobile, array('shop_name' => $detail['shop_name'], 'name' => $data['name'], 'content' => $data['content'], 'yuyue_time' => $data['yuyue_time'], 'mobile' => $data['mobile'], 'number' => $data['number'], 'yuyue_date' => $data['yuyue_date']));
                        }
                    }
                }
    ...
    public function shop(){
        ...
        if (empty($branch_id)) {
            ...
            $ex = D('Shopdetails')->changeBTime($ex);
    ...
    public function recognition(){
        ...
        if ($this->isPost()) {
            $data = $this->checkFields($this->_post('data', false), array('name', 'country', 'mobile', 'content'));
            ...
            if ($data['country'] == 'nz') {
                if (!isMobile($data['mobile'])) {
                    $this->fengmiMsg('请输入正确的NZ手机号码');
                }
            } elseif ($data['country'] == 'cn') {
                if (!isCnMobile($data['mobile'])) {
                    $this->fengmiMsg('请输入正确的中国手机号码');
                }
            }
            $data['content'] = htmlspecialchars($data['content']);
            if (empty($data['content'])) {
            ...
            if ($obj->add($data)) {
                ...
                if (isCnMobile($mobile)) {
                    D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_shop_recognition_admin', $mobile, array('shop_name' => $detail['shop_name'], 'name' => $data['name']));
                } elseif (isMobile($mobile)) {
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_shop_recognition_admin', $mobile, array('shop_name' => $detail['shop_name'], 'name' => $data['name']));
                    } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {
