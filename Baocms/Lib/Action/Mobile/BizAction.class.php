...
class BizAction extends CommonAction {
	...
	public function gps($pois_id){
        ...
        //如果是入驻商家
        if (!empty($Biz['shop_id'])) {
            $shop = D('Shop')->find($Biz['shop_id']);
        ...
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                ...
                if (isCnMobile($data['mobile'])) {
                    D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_shop_yuyue_code', $data['mobile'], array(
                    ...
                    if($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_shop_yuyue_code', $data['mobile'], array(
                            'shop_name' => $detail['name'],
                            'shop_tel' => $shops['mobile'],
                            'shop_addr' => $detail['address'],
                            'code' => $data['code']
				...
				//预约通知商家功能开始
               if (isCnMobile($shops['mobile'])) {
                   D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_shangjia_yuyue', $shops['mobile'], array(
               ...
               } elseif (isMobile($shops['mobile'])) {
                   if($this->_CONFIG['sms']['dxapi'] == 'bu') {
                       D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_shangjia_yuyue', $shops['mobile'], array(
                           'sitename' => $this->_CONFIG['site']['sitename'],
                           'name' => $data['name'],
                           'content' => $data['content'],
                           'yuyue_time' => $data['yuyue_time'],
                       ...
                       D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_shangjia_yuyue', $shops['mobile'], array(
                           'name'=>$data['name'],
                           'content'=>$data['content'],
    ...
    public function checkBook() {
        $data = $this->checkFields($this->_post('data', false), array('pois_id','name', 'country', 'mobile','type', 'content', 'yuyue_date', 'number'));
        ...
        if ($data['country'] == 'nz') {
            if (!isMobile(htmlspecialchars($data['mobile']))) {
                $this->fengmiError('请输入正确的NZ手机号码');
            }
        } elseif ($data['country'] == 'cn') {
            if (!isCnMobile(htmlspecialchars($data['mobile']))) {
                $this->fengmiError('请输入正确的中国手机号码');
            }
        }
