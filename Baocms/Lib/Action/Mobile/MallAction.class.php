...
class MallAction extends CommonAction{
    ...
    public function cart(){
        ...
        $cart_shops = D('Shop')->itemsByIds($shop_ids);
        $has_direct_sale = false;
        foreach ($cart_shops as $k => $v) {
            if ($v['is_direct_sale'] == 1) {
                $has_direct_sale = true;
            }
        }
        $this->assign('collection_date', collection_date());
        $this->assign('has_direct_sale', $has_direct_sale);
        $this->assign('cart_shops', $cart_shops);
    ...
    public function detail($goods_id) {
        ...
        $wd= D('WeidianDetails')->field('id')->where(array('shop_id' => $shop_id,'audit'=>1))->find();
        if (empty($wd['id'])) {
            $this->error('该店不存在!');
        }
      
        if (strstr($_SERVER['HTTP_REFERER'],'/mobile/mall/index')) {
            $ReturnAddress=U('mall/index');
       }else{
            $ReturnAddress=U('mart/lists',array('id'=>$wd['id']));
       }
        ...
        $this->assign('ReturnUrl', $ReturnAddress);
    ...
    public function order() {
        ...
        //订单驿站站长记录
        $zhanzhang = D('Users')->field('tui_zhanzhang,bang_zhanzhang')->where(array('user_id'=>$this->uid))->find();
        $tui_zhanzhang = $zhanzhang['tui_zhanzhang'];
        $bang_zhanzhang = $zhanzhang['bang_zhanzhang'];
        ($tui_zhanzhang > 0) && $order['tui_zhanzhang'] = $tui_zhanzhang;
        ($bang_zhanzhang > 0) && $order['bang_zhanzhang'] = $bang_zhanzhang;
        $collection_time = $this->_post('collection');
        $order_ids = array();
        foreach ($ordergoods as $k => $val) {
            $expressprice=D('Weidiandetails')->where(array('shop_id' => $k))->getfield('express_price'); //生成订单时候，按照订单生成运费
            $order['shop_id'] = $k;
            ...
            $order['total_express'] = $expressprice;  // 增加

            if ($collection_time[$k] && in_array($collection_time[$k], collection_date())) {
                $order['collection_time'] = $collection_time[$k];
            }
            if(!D('Order')->autoCheckToken($_POST)){
                $this->fengmiMsg('亲！，订单已存在！', U('../mcenter/goods'));
            }
        ...
        if (count($order_ids) > 1) {
            $orderexpress =D('Order')->field('sum(total_express) as total_express')->where(array('order_id' => array('IN', $order_ids)))->find();
            $need_pay = D('Order')->useIntegral($this->uid, $order_ids);
            $need_pay +=$orderexpress['total_express']*100;
            //扣优惠券优惠金额
            $total_coupon_price =0;
            foreach ($order_ids as $k => $val) {
                $list[$val] = D('Coupon')->Obtain_Coupon($val,$this->uid);
                $total_coupon_price += $list[$val]['reduce_price'];
            }
            $need_pay = $need_pay - $total_coupon_price;
    ...
    public function paycode(){
        ...
        // 计算订单下所有店的运费
        $shopid= D('Ordergoods')->where(array('order_id' => array('IN', $order_ids)))->getfield('GROUP_CONCAT(DISTINCT `shop_id`)'); 
        $shop_id= explode(',', $shopid);
        $totalexpressfee =D('Weidiandetails')->where(array('shop_id' => array('IN', $shop_id)))->getfield('sum(express_price)');
        $use_integral= D('Order')->where(array('order_id' => array('IN', $order_ids)))->getfield('sum(use_integral)'); 
        ...
        $detail['total_express']=$totalexpressfee;
        $detail['use_integral']=$use_integral;
        ...
        $this->assign('reduce_price', $total_coupon_price); // 使用所有优惠券
    ...
    public function pay(){
        ...
        // 计算订单下所有店的运费
        $shopid= D('Ordergoods')->field('shop_id')->group('shop_id')->where(array('order_id' => $order_id))->find();
        $map['shop_id']  = array('in',$shopid);
        $totalexpressfee =D('Weidiandetails')->where($map)->getfield('sum(express_price)');
        ...
        $order['total_express']=$totalexpressfee;
    ...
    public function paycode2(){
        ...
        foreach ($order_ids as $k => $val) {
            $list[$val] = D('Coupon')->Obtain_Coupon($val,$this->uid);
            D('Order')->where(array('order_id' => $val))->save(array('download_id' => $list[$val]['download_id'],'coupon_price' => $list[$val]['reduce_price']));
        }
        ...
        foreach ($orders as $k => $val) {
            $need_pay[$val[order_id]] = $val['total_price']+$val['total_express']*100- $val['mobile_fan'] - $val['use_integral']-$val['coupon_price']; // 这里以前版本没有考虑运费，多店没考虑减优惠券
            D('Order')->where(array('order_id' => $val['order_id']))->save(array('need_pay' => $need_pay[$val[order_id]])); // 生成订单后，跳转到支付前更新了needpay...
        }
        /*****************************************************/
    ...
    public function combine(){
        ...
        // 商城购物 才有积分抵现 & 只有多店调用此功能
		if  ($detail['type']='goods') {
			if ($detail['order_id']==0 && !empty($detail['order_ids'])) { 
               $order =D('Order')->field('sum(total_express) as total_express,sum(can_use_integral) as can_use_integral,sum(use_integral) as use_integral ')->where(array('order_id' => array('IN', $order_id)))->find();
            }
        }

        $detail['total_express'] = $order['total_express'];
        $detail['can_use_integral'] = $order['can_use_integral'];
        $detail['use_integral'] = $order['use_integral'];
    ...
    public function pay2(){
        ...
        if ($code == 'wait') {
            ...
            $express = $order['total_express']; 
            $need_pay +=  $express*100;
            //更新支付结果
            if (empty($logs)) {
                $logs = array(
					'type' => 'goods', 
