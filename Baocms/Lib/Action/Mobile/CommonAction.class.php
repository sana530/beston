...
class CommonAction extends Action{
    ...
    protected function _initialize(){
        ...
        //驿站站长推广
        $tui_zhanzhang = (int) $this->_get('tuizhanzhang');
        if ($tui_zhanzhang) {
            cookie('tui_zhanzhang', $tui_zhanzhang, 30 * 86400);
        }
        ...
        // 是否禁用定位功能的页面
        $disable_location = explode('|', strtolower($this->_CONFIG['site']['disable_location']));
        $request_uri = strtolower($_SERVER["REQUEST_URI"]);
        $num = count($disable_location);
        for($i=0;$i<$num;++$i){ 
            if(stripos($request_uri,$disable_location[$i]) >0 ){
               $this->assign('Disable', 1);
            }
        } 

        // 增加微信分享的功能
        import("@/Net.Jssdk2017");
        $jssdk = new Jssdk2017($this->_CONFIG['weixin']["appid"],$this->_CONFIG['weixin']["appsecret"]);
        ...
        $this->assign('city_chinese_name', $city['chinese_name']);
        //您当前可能在的城市
    ...
    protected function check_mobile()
    {
        ...
        if ($m == null || (!isMobile($m) && !isCnMobile($m))) {
            $mobile_open = 0;
        } else {
            $mobile_open = 1;
        }
    ...
    protected function life_yzm()
    {
        ...
        $this->ajaxReturn(array('status' => 'success', 'msg' => '短信发送成功，请留意收到的手机短信！', 'code' => session('life_code')));
    }
