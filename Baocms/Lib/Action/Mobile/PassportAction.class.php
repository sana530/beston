...
class PassportAction extends CommonAction
{
    ...
    {
        if ($this->isPost()) {
            if ($_POST['data']['country'] == 'nz') {
                if (!isMobile(htmlspecialchars($_POST['account']))) {
                    $this->fengmiMsg('请输入正确的NZ手机号码');
                }
            } elseif ($_POST['data']['country'] == 'cn') {
                if (!isCnMobile(htmlspecialchars($_POST['account']))) {
                    $this->fengmiMsg('请输入正确的中国手机号码');
                }
            }
            if (isMobile(htmlspecialchars($_POST['account'])) || isCnMobile(htmlspecialchars($_POST['account']))) {
                if (!($scode = trim($_POST['scode']))) { $this->fengmiMsg('请输入短信验证码！'); }
                $scode2 = session('scode');
                if (empty($scode2)) { $this->fengmiMsg('请获取短信验证码！'); }
                if ($scode != $scode2) { $this->fengmiMsg('请输入正确的短信验证码！'); }
            }
            $data = $this->createCheck();
            $invite_id = (int) cookie('invite_id');
            if (!empty($invite_id)) { $data['invite_id'] = $invite_id; }
            $password2 = $this->_post('password2');
            if ($password2 !== $data['password']) { $this->fengmiMsg('两次密码不一致'); }
            //开始其他的判断了
            if (true == D('Passport')->register($data)) { $this->fengmiMsg('恭喜您，注册成功！', U('index/index')); }
    ...
    private function createCheck()
    {
        ...
        if (empty($data['nickname']) || $data['nickname'] == '请输入您的用户名（可选）') {
    ...
    public function sendsms()
    {
        ...
        if ($_POST['country'] == 'nz') {
            if (!isMobile($mobile)) { die('请输入正确的NZ手机号码'); }
        } elseif ($_POST['country'] == 'cn') {
            if (!isCnMobile($mobile)) { die('请输入正确的中国手机号码'); }
        }
        if ($user = D('Users')->getUserByMobile($mobile) || $user = D('Users')->getUserByAccount($mobile)) {
            die('手机号码已经存在！');
        }
        ...
        if (isCnMobile($mobile)) {
            D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_yzm', $mobile, array('sitename' => $this->_CONFIG['site']['sitename'], 'code' => $randstring));
        } elseif (isMobile($mobile)) {
            if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_yzm', $mobile, array('sitename' => $this->_CONFIG['site']['sitename'], 'code' => $randstring));
    ...
    public function newpwd()
    {
        ...
        if ($way == 1) {
            ...
            D('Passport')->uppwd($user['account'], '', $password);
            if (isCnMobile($mobile)) {
                D('Sms')->CnSms($this->_CONFIG['site']['sitename'], 'sms_user_newpwd', $mobile, array('sitename' => $this->_CONFIG['site']['sitename'], 'newpwd' => $password));

            } elseif (isMobile($mobile)) {
                if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                    D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_user_newpwd', $mobile, array('sitename' => $this->_CONFIG['site']['sitename'], 'newpwd' => $password));
    ...
    {
        if (!($mobile = htmlspecialchars($_POST['mobile']))) { die('请输入手机号码'); }
        if ($_POST['country'] == 'nz') {
            if (!isMobile($mobile)) { die('请输入正确的NZ手机号码'); }
        } elseif ($_POST['country'] == 'cn') {
            if (!isCnMobile($mobile)) { die('请输入正确的中国手机号码'); }
        }
        /*if (!($account = htmlspecialchars($_POST['account']))) { die('请填写账号'); }
        if ($user = D('Users')->getUserByAccount($account)) {
            if (empty($user['mobile'])) {
                die('你还未绑定手机号，请选择其他方式！');
            } else {
                if ($user['mobile'] != $mobile) { die('请填写您的绑定手机号！'); }
            }
        }*/
        $randstring = session('scode');
        if (empty($randstring)) {
    ...
    public function payPassword(){
        ...
        $pwd = $this->_post('passwd');
        $type = $this->_post('type');
        if (($type==0) && (strlen($pwd) != 4) && !empty($pwd)) {
            echo 0;die;
        }
        echo D('Passport')->checkPay($pwd, $type);
    }
    ...
    public function scanlogin () {
        $backurl = base64_encode('http://' . $_SERVER['HTTP_HOST'] . U('passport/cbscan'));
        $url = 'http://pay.1stopbuild.com/mobile/scan/login/key/'.$this->_CONFIG['site']['1stpay_scan_login'].'/backurl/'.$backurl;
        header('Location:'.$url);
    }
     ...
     * 1stpay扫一扫登陆回传结果处理
     */
    public function cbscan () {
        //取得CURL返回的用户信息
        $token = $this->_get('token');
        $url = 'https://pay.1stopbuild.com/mobile/scan/getinfo/key/'.$this->_CONFIG['site']['1stpay_scan_login'].'/token/'.$token;
        $res = file_get_contents_url($url);
        //解密用户id (只有1stpay才能对user_id进行解密)
        $user_id = D('Crypter')->decrypt($res->data->user_id);
        //检查用户状态
        $checkuser = D('Users')->where(array('user_id'=>$user_id))->getField('closed');
        if ($checkuser==NULL || $checkuser!=0) {
            $this->error('用户状态不正常，请联系1stPay客服');
        }
        //成功登陆后，跳转到用户中心页
        setUid($user_id);
        redirect(U('Mcenter/member/index'),0);
    }
}