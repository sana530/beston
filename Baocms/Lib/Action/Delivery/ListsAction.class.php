            ...
            if(!cookie('DL')){
                ...
                if(!$f){
                    ...
                    if($up){
                        ...
                        $User = D('Users') -> where('user_id ='.$f['user_id']) -> find();
						if (isMail($User['email'])) {
                            D('Email')->sendMail(
                                'email_tz_delivery',
                                $User['email'],
                                $this->_CONFIG['site']['sitename'].'物流已被配送',
                                array('sitename' => $this->_CONFIG['site']['sitename'], 'tel' => $this->_CONFIG['site']['tel'])
                            );
                        }
        ...
        if ( IS_AJAX ){
            ...
            if ( !cookie( "DL" ) ){
                ...
                if ( FALSE !== $express->save( $data ) ){
                    $User = D('Users') -> where('user_id ='.$detail['user_id']) -> find();
                    if (isMail($User['email'])) {
                        D('Email')->sendMail(
                            'email_tz_delivery',
                            $User['email'],
