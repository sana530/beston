...
class EleAction extends CommonAction{
    ...
    public function create()
    {
        if (IS_AJAX) {
            ...
            $obj = D('Elecate');
            $data = array('shop_id' => $shop_id, 'cate_name' => $cate_name, 'closed' => 0);
            $data['booking_cate_id'] = D('Bookingcate')->add($data);    //同时添加订座菜系
            $data['num'] = 0;
            if ($obj->add($data)) {
        ...
        if (IS_AJAX) {
            ...
            if ($cate_id) {
                ...
                if (false !== $obj->where('cate_id =' . $cate_id)->setField($data)) {
                    //同时修改订座菜系
                    $data['cate_id'] = $detail['booking_cate_id'];
                    if (false !== D('Bookingcate')->save($data)) {
    ...
    public function eleorder_lada_data(){
        ...
        $this->assign('list', $list);
        $this->assign('page', $show);
        $this->assign('deliverytypes', D('Eleorder')->getDeliveryTypes());
		$this->display(); // 输出模板
    }
    ...
    public function delete($product_id = 0){
        ...
        D('Bookingmenu')->save(array('menu_id' => $detail['booking_menu_id'], 'closed' => 1));  //同时删除订座菜单
    ...
    public function updates($product_id = 0){
        ...
        D('Bookingmenu')->save(array('menu_id' => $detail['booking_menu_id'], 'closed' => 0));  //同步上架成功
    ...
    public function create2()
    {
        if ($this->isPost()) {
            ...
            //插入订座菜单
            $booking_cate_id = D('Elecate')->where(array('cate_id'=>$data['cate_id']))->getField('booking_cate_id');
            $booking_add['cate_id'] = $booking_cate_id;
            $booking_add['menu_name'] = $data['product_name'];
            $booking_add['shop_id'] = $data['shop_id'];
            $booking_add['photo'] = $data['photo'];
            $booking_add['price'] = $data['price'];
            $booking_add['ding_price'] = $data['price'];
            $booking_add['sold_num'] = 0;
            $booking_add['is_tuijian'] = $data['is_tuijian'];
            $booking_add['is_new'] = $data['is_new'];
            $booking_add['is_sale'] = 0;
            $booking_add['create_time'] = NOW_TIME;
            $booking_add['create_ip'] = get_client_ip();
            $booking_add['closed'] = 0;
            $data['booking_menu_id'] = D('Bookingmenu')->add($booking_add);
            if (false === $data['booking_menu_id']) {
                $this->error('操作失败');
            }
    ...
    public function edit2($product_id = 0)
    {
        if ($product_id = (int) $product_id) {
            ...
            if ($this->isPost()) {
                ...
                //同时修改订座菜单
                $booking_save['menu_id'] = $detail['booking_menu_id'];
                $booking_cate_id = D('Elecate')->where(array('cate_id'=>$data['cate_id']))->getField('booking_cate_id');
                $booking_save['cate_id'] = $booking_cate_id;
                $booking_save['menu_name'] = $data['product_name'];
                $booking_save['photo'] = $data['photo'];
                $booking_save['price'] = $data['price'];
                $booking_save['ding_price'] = $data['price'];
                $booking_save['is_tuijian'] = $data['is_tuijian'];
                $booking_save['is_new'] = $data['is_new'];
                if (false === D('Bookingmenu')->save($booking_save)) {
                    $this->error('操作失败');
                }
