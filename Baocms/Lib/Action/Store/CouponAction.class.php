...
class CouponAction extends CommonAction {
    ...
    private $create_fields = array( 'title', 'photo', 'expire_date','num','full_price','reduce_price','limit_num', 'intro');
    private $edit_fields = array( 'title', 'photo', 'expire_date','num','full_price','reduce_price','limit_num', 'intro');
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $obj = D('Coupon');
            if ($obj->add($data)) {
                $this->fengmiMsg('添加成功!', U('store/coupon/sale'));
            }
            $this->fengmiMsg('操作失败！');
        } else {
            $this->display();
