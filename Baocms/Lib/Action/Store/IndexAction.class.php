...
class IndexAction extends CommonAction {
    ...
    public function index() {
		...
		//分类信息
		$counts['life'] = (int) D('Life')->where(array('user_id' => $this->uid,'closed' => 0))->count();//总分类信息
		$counts['life_audit'] = (int) D('Life')->where(array('user_id' => $this->uid,'audit' => 0,'closed' => 0))->count();//待审核分类信息
		//商家招聘
