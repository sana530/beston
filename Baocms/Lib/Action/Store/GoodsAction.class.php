...
{
    //后期增加添加商品
    private $create_fields = array('title', 'photo', 'cate_id', 'intro', 'guige', 'num', 'select1', 'select2', 'select3', 'select4', 'select5', 'price', 'shopcate_id', 'mall_price','is_agent_price', 'settlement_price', 'use_integral', 'commission', 'instructions', 'details', 'end_date', 'is_vs1', 'is_vs2', 'is_vs3', 'is_vs4', 'is_vs5', 'is_vs6', 'photo_id', 'photo_dt');
    private $edit_fields = array('title', 'photo', 'cate_id', 'intro', 'guige', 'num', 'select1', 'select2', 'select3', 'select4', 'select5', 'price', 'shopcate_id', 'mall_price','is_agent_price', 'settlement_price', 'use_integral', 'commission', 'instructions', 'details', 'end_date', 'is_vs1', 'is_vs2', 'is_vs3', 'is_vs4', 'is_vs5', 'is_vs6', 'audit', 'photo_id', 'photo_dt');
    ...
    public function weidian()
    {
        ...
        if ($this->isPost()) {
            $data = $this->checkFields($this->_post('data', false), array('weidian_name', 'addr', 'city_name', 'area_name', 'cate_id', 'business_time', 'details', 'pic', 'lng', 'lat', 'reg_time'));
            if (empty($weidian)) {
                ...
                if (empty($data['city_name']) || empty($data['area_name'])) {
            ...
            if (!$weidian) {
                ...
                if (!$add) {
                    ...
                    $this->fengmiMsg('设置成功', U('mart/index'));
                }
            } else {
                //否则修改
    ...
    public function create()
    {
        ...
        if ($this->isPost()) {
            ...
            if ($goods_id = $obj->add($data)) {
                ...
                $this->fengmiMsg('添加成功', U('store/mart/index'));
            }
            $this->fengmiMsg('操作失败！');
        } else {
    ...
    private function createCheck()
    {
        ...
        if (empty($data['is_agent_price'])) {
            $this->fengmiMsg('代理商价格不能为空');
        }
        if ($data['is_agent_price'] >= $data['mall_price']) {
            $this->fengmiMsg('代理商价格不能大于等于普通会员的价格');
        }
        ...
        if ($goods_id = (int) $goods_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->fengmiMsg('编辑成功', U('store/mart/index'));
                }
                $this->fengmiMsg('操作失败');
            } else {
