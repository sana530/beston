...
class BookingmenuAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $obj = D('Bookingmenu');
            if ($obj->add($data)) {
                $this->fengmiMsg('添加成功', U('bookingmenu/index'));
            }
            $this->fengmiMsg('操作失败！');
        } else {
            $this->display();
        ...
        if ($menu_id = (int) $menu_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->fengmiMsg('操作成功', U('bookingmenu/index'));
                }
                $this->fengmiMsg('操作失败');
            } else {
    ...
    public function delete($menu_id = 0) {
        if (is_numeric($menu_id) && ($menu_id = (int) $menu_id)) {
            ...
            $this->ajaxReturn(array('status'=>'success','msg'=>'删除成功', U('store/bookingmenu/index')));
        }
        $this->ajaxReturn(array('status'=>'error','msg'=>'访问错误！'));
    }
	 ...
	 public function updates($menu_id = 0) {
        if (is_numeric($menu_id) && ($menu_id = (int) $menu_id)) {
            ...
            $this->ajaxReturn(array('status'=>'success','msg'=>'上架成功', U('store/bookingmenu/shelves')));
        }
        $this->ajaxReturn(array('status'=>'error','msg'=>'访问错误！'));
    }
