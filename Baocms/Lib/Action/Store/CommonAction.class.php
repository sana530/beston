...
class CommonAction extends Action{
    ...
    protected function _initialize(){
        ...
        if (strtolower(MODULE_NAME) != 'passport' && strtolower(MODULE_NAME) != 'public') {
            ...
            $this->shop = D('Shop')->find(array("where" => array('user_id' => $this->uid)));
            if (empty($this->shop)) {
                $this->error('该用户没有开通商户。', U('passport/login'));
            } elseif ($this->shop['closed'] == 1) {
                $this->error('您的商铺已被关闭，请联系管理员。', U('passport/login'));
            } elseif ($this->shop['audit'] == 0) {
                $this->error('您的商铺正在被审核，请耐心等待。', U('passport/login'));
            }
            $this->shop_id = $this->shop['shop_id'];
            $this->assign('SHOP', $this->shop);
        }
        //为安全考虑，第一次打开商家页面后，将uid cookie改为两小时过期
        if (!cookie('shangjia_login')) {
            clearUid();
            setUid($this->uid, 2);
            cookie('shangjia_login', 1, 2*3600);
        }
