...
class EleorderModel extends CommonModel {
    ...
    protected $isDeliveryStatus = array(
        0 => '自主取餐',
        1 => '配送',
    );
            ...
            if ($detail['is_pay'] == 1) {
                ...
                if ($detail['is_delivery'] == 1) {
                    if($shop['is_pei'] == 0){
                        ...
                        $money = $settlement_price + $ele['logistics'] - $config['mobile']['delivery_price'] * 100;
                    }else{
                        $money = $settlement_price + $ele['logistics'];
                    }
                } else {
                    $money = $settlement_price; //如果不配送，则结算的时候不考虑运费
                }
