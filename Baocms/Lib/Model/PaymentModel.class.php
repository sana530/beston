            ...
            if (D('Paymentlogs')->save($data)) {
                ...
                if ($logs['type'] == 'breaks') {//优惠买单
                    ...
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_hotel', $order['mobile'], array(
                        ...
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_hotel', $order['mobile'], array(
                            'hotel_name' => $hotel['hotel_name'],
                            'addr'=>$hotel['addr'],
                            'tel' => $hotel['tel'],
                            'stime' => $order['stime'],
                    ...
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_shop_hotel', $shop['mobile'], array());
                    } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_shop_hotel', $shop['mobile'], array());
                    } else {
                    ...
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_farm', $order['mobile'], array(
                        ...
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_farm', $order['mobile'], array(
                            'farm_name' => $farm['farm_name'],
                            'addr'=>$farm['addr'],
                            'tel' => $farm['tel'],
                            'gotime' => $order['gotime'],
                    ...
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_shop_farm', $shop['mobile'], array());
                    } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_shop_farm', $shop['mobile'], array());
                    } else {
                    ...
                    if ($shops['is_tuan_print'] == 1) {
                        //打印
                        $msg = D('Tuanorder')->tuan_print($order['order_id'], $order['addr_id']);
	                ...
	                D('Tongji')->log(4, $logs['need_pay']); //统计
					D('Sms')->sms_booking_user($logs['order_id']);//短信通知会员
					D('Sms')->sms_booking_shop($logs['order_id']);//通知商家
                    $order = D('Bookingorder')->where('order_id =' . $logs['order_id'])->find();
                    $shops = D('Shop')->find($order['shop_id']);
                    $shop_id = $shops['shop_id'];
                    if ($shops['is_ding_print'] == 1) {
                        //打印
                        $msg = D('Bookingorder')->ding_print($order['order_id'], $order['addr_id']);
                        $result = D('Print')->printOrder($msg, $shop_id);
                        Log::record('打印订单结果：' . $result);
                    }
					D('Weixintmpl')->weixin_pay_balance_user($logs['log_id']);//会员账户余额变动通知全局
                    ...
                    //短信通知订座
                    if ($this->_CONFIG['sms']['dxapi'] == 'bu') {
                        D('Sms')->BuSms($this->_CONFIG['site']['sitename'], 'sms_ding', $member['mobile'], array('nickname' => $member['nickname'], 'shopname' => $shops['shop_name']));
                    } elseif ($this->_CONFIG['sms']['dxapi'] == 'dy') {
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_ding', $member['mobile'], array('nickname' => $member['nickname'], 'shopname' => $shops['shop_name']));
                    } else {
                ...
                } elseif ($logs['type'] == 'api') {
                    D('Scanorder')->save(array('order_id' => $logs['order_id'], 'status' => 1, 'paid_time'=>NOW_TIME, 'paid_ip'=>get_client_ip()));
                    $appid = D('Scanorder')->where(array('order_id'=>$logs['order_id']))->getField('appid');
                    D('Scanapikey')->where(array('appid'=>$appid))->setInc('num_count');
                    D('Weixintmpl')->weixin_pay_balance_user($logs['log_id']);//会员账户余额变动通知全局
                    D('Tongji')->log(5, $logs['need_pay']);//统计
                } elseif ($logs['type'] == 'receipt') {
                    $receiptOrder = D('Shopreceiptorder');
                    $receiptOrder->save(array('order_id' => $logs['order_id'], 'status' => 1, 'paid_time'=>NOW_TIME, 'paid_ip'=>get_client_ip()));
                    $receiptOrder->overOrder($logs['order_id']);
                    D('Weixintmpl')->weixin_pay_balance_user($logs['log_id']);//会员账户余额变动通知全局
                    D('Weixintmpl')->weixin_pay_balance_shop($logs['log_id'],1);//微信通知商家
                    D('Tongji')->log(6, $logs['need_pay']);//统计
                } else {
                    // 商城购物
                    if (empty($logs['order_id']) && !empty($logs['order_ids'])) {
                        ...
                        foreach($order_ids as $order_id){
                            D('Coupon')->change_download_id_is_used($order_id);//如果有优惠劵就修改优惠劵的状态
                        }
                        ...
                        foreach ($order_ids as $order_id) {
                            $order_info = D('Order')->field('addr_id, shop_id')->where('order_id =' . $order_id)->find();
                            $addr_id = $order_info['addr_id'];
                            $shop_id = $order_info['shop_id'];
                            //打印小票
                            $is_goods_print = D('Shop')->where(array('shop_id'=>$shop_id))->getField('is_goods_print');
                            if ($is_goods_print == 1) {
                                $msg = D('Order')->goods_print($order_id, $addr_id);
                                $result = D('Print')->printOrder($msg, $shop_id);
                                Log::record('打印订单结果：' . $result);
                            }
                        }
                        ...
                        $addr_id = D('Order')->where('order_id =' . $logs['order_id'])->getField('addr_id');
                        $shop_id = $goods_order_shop['shop_id'];
                        if ($goods_order_shop['is_goods_print'] == 1) {
                            //打印
                            $msg = D('Order')->goods_print($logs['order_id'], $addr_id);
                            $result = D('Print')->printOrder($msg, $shop_id);
                            Log::record('打印订单结果：' . $result);
                        }
                    }
