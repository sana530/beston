...
class SmsModel extends CommonModel{
    ...
    public function BuSms($sign, $code, $mobile, $data){

        $burstcode = D('Burst')->where(array("burst_key='{$code}'"))->find();
        if (!empty($burstcode['is_open'])) {
            $sms_id = $this->sms_burst_add($sign, $code, $mobile,$data,$burstcode['burst_tmpl']);
            $content = $burstcode['burst_tmpl'];
            $config = D('Setting')->fetchAll();
            $data['sitename'] = $config['site']['sitename'];
            $data['tel'] = $config['site']['tel'];
            foreach ($data as $k => $val) {
            ...
            import('ORG.Util.Burst');
            $obj = new transmitSms($config['sms']['bukey'], $config['sms']['busecret']);
            $backresult=$obj->sms_id($sms_id)->sendSms($content, preg_replace('/^[0]{1,}/','64',$mobile));
    ...
    public function CnSms($sign, $code, $mobile, $data){
        ...
        $burstcode = D('Burst')->where(array("burst_key='{$code}'"))->find();
        if (!empty($burstcode['is_open'])) {
            $sms_id = $this->sms_cn_add($sign, $code, $mobile, $data, $burstcode['burst_tmpl']);
            $content = $burstcode['burst_tmpl'];
            $config = D('Setting')->fetchAll();
            $data['sitename'] = $config['site']['sitename'];
            $data['tel'] = $config['site']['tel'];
            ...
            $re = trim($this->postSMS($http, $data2)); // POST方式提交
            if($re == '100' || substr($re, 3, 3) == '100'){
                D('Cnsms')->where(array('sms_id' => $sms_id))->save(array('status' => 1, 'info'=>"接收成功"));
                return true;
            } else {
                D('Cnsms')->where(array('sms_id' => $sms_id))->save(array('status' => 0, 'info'=>"验证失败"));
            }
        }
        return false;
    }
    ...
    public function sms_burst_add($sign, $code, $mobile,$data,$burst_note){
        foreach ($data as $k => $val) {
            $content = str_replace('{' . $k . '}', $val, $burst_note);
            $burst_note = $content;
        }
        $sms_data = array();
        $sms_data['sign'] = $sign.'-'.time();
        ...
        $sms_data['create_time'] = time();
        $sms_data['create_ip'] = get_client_ip();
        if ($sms_id = D('Burstsms')->add($sms_data)) {
            return $sms_id;
        }
        return true;
    }

    public function sms_cn_add($sign, $code, $mobile,$data,$burst_note){
        foreach ($data as $k => $val) {
            $content = str_replace('{' . $k . '}', $val, $burst_note);
            $burst_note = $content;
        }
        ...
        $sms_data['create_time'] = time();
        $sms_data['create_ip'] = get_client_ip();
        if ($sms_id = D('Cnsms')->add($sms_data)) {
            return $sms_id;
        }
        return true;
    }
    ...
    public function mallTZshop($order_id){
        ...
        foreach ($shops as $val) {
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'sms_mall_tz_shop',$val['mobile'], array(
                    'sitename' => $config['site']['sitename']
                ));
            }elseif ($config['sms']['dxapi'] == 'dy') {
	...
	public function coupon_download_user($download_id,$uid){
		 ...
		 if(!empty($user['mobile'])){
             if ($config['sms']['dxapi'] == 'bu') {
                D('Sms')->BuSms($config['site']['sitename'], 'coupon_download_user', $user['mobile'], array(
    ...
    public function eleTZshop($order_id){
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'sms_ele_tz_shop',$shop['mobile'], array(
                    'sitename' => $config['site']['sitename'],
                    'sitename' => $config['site']['sitename']
                ));
            }elseif ($config['sms']['dxapi'] == 'dy') {
	...
	public function breaksTZshop($order_id){
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
			...
			if(!empty($shop['mobile'])){
                if ($config['sms']['dxapi'] == 'bu') {
                    $this->BuSms($config['site']['sitename'], 'sms_breaks_tz_shop',$shop['mobile'], array(
	...
	public function breaksTZuser($order_id){
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
			...
			if(!empty($users['mobile'])){
                if ($config['sms']['dxapi'] == 'bu') {
                    $this->BuSms($config['site']['sitename'], 'sms_breaks_tz_user',$users['mobile'], array(
                        'user_name' => $user_name, //会员名字
                        'shop_name' => $shop['shop_name'], //商家名字
					...
					$this->sendSms('sms_breaks_tz_user', $users['mobile'], array(
						'user_name' => $user_name, //会员名字
						'shop_name' => $shop['shop_name'], //商家名字
						'money' => $order['need_pay'], //实付金额
						'data' => $date, //买单时间
    ...
    public function tuan_TZ_user($code_id){
        if (is_numeric($code_id) && ($code_id = (int) $code_id)) {
            ...
            $date = date('Y-m-d H:i:s ', $t);
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'],'tuan_TZ_user',$user['mobile'], array(
                    'name' => $tuan['title'],
                    'data' => $date,
                    'tel' => $config['site']['tel']
                ));
	...
	public function sms_tuan_user($uid,$order_id){
		...
		$tuan = D('Tuan')->find($order[0]['tuan_id']);
		foreach($order as $v){
			$code[] =  $v['code'];
		}
		$codestr = join(',', $code);
        //发送团购劵
         if ($config['sms']['dxapi'] == 'bu') {
             D('Sms')->BuSms($config['site']['sitename'], 'sms_tuan_user',$user['mobile'], array(
                 'code' => $codestr,
                 'user' => $user['nickname'],
    ...
    public function dingTZshop($order_id)
    {
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            $shop = D('Shop')->find($order['shop_id']);
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'sms_ele_shop',$shop['mobile'], array('sitename' => $config['site']['sitename']));
            }elseif ($config['sms']['dxapi'] == 'dy') {
    ...
    public function tuanTZshop($shop_id)
    {
        ...
        $config = D('Setting')->fetchAll();
        if ($config['sms']['dxapi'] == 'bu') {
            $this->BuSms($config['site']['sitename'], 'sms_tuan_tz_shop',$shop['mobile'], array('sitename' => $config['site']['sitename']));
        }elseif ($config['sms']['dxapi'] == 'dy') {
	...
	public function sms_booking_user($order_id){
			...
			$config = D('Setting')->fetchAll();
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'sms_booking_user',$member['mobile'], array(
	...
	public function sms_booking_shop($order_id){
			...
			$config = D('Setting')->fetchAll();
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'sms_booking_shop',$shops['mobile'], array(
                    'booking_name' => $booking['shop_name'],//预订名字
                ));
            }elseif ($config['sms']['dxapi'] == 'dy') {
	...
	public function sms_crowd_user($order_id){
			...
			$config = D('Setting')->fetchAll();
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'sms_crowd_user',$users['mobile'], array(
	...
	public function sms_crowd_uid($order_id){
			...
			$config = D('Setting')->fetchAll();
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'sms_crowd_uid',$users['mobile'], array(
	...
	public function goods_refund_user($order_id){
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            $date = date('Y-m-d H:i:s ', $t);
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'goods_refund_user',$user['mobile'], array(
	...
	public function eleorder_refund_user($order_id){
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            $date = date('Y-m-d H:i:s ', $t);
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'eleorder_refund_user',$user['mobile'], array(
	...
	public function tuancode_refund_user($code_id){
            ...
            $user = D('Users')->find($Tuancode['user_id']);
            if ($config['sms']['dxapi'] == 'bu') {
                $this->BuSms($config['site']['sitename'], 'tuancode_refund_user',$user['mobile'], array(
