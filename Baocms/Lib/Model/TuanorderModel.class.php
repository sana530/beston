...
class TuanorderModel extends CommonModel{
    ...
    public function tuan_print($order_id,$addr_id) {
        ...
        $msg .= '@@2团购清单__________NO:' . $order['order_id'] . '\r';
        $msg .= '店名：' . $shop_print['shop_name'] . '\r';
        $msg .= '联系人：' . $member['name'] . '\r';
        $msg .= '电话：' . $member['mobile'] . '\r';
        $msg .= '商家地址：' . $shop_print['addr'] . '\r';
        $msg .= '商家电话：' . $shop_print['tel'] . '\r';
        $msg .= '----------------------\r';
        $msg .= '@@2团购商品明细\r';
        $products = D('Tuan')->where(array('tuan_id' => $order['tuan_id']))->find();
        $msg .= '团购标题：' . $products['title'] . '\r';
        $msg .= '原价：' . $products['price']/100 .'元' . '\r';
        $msg .= '团购价：' . $products['tuan_price']/100 .'元' . '\r';
        $msg .= '----------------------\r';
        $msg .= '@@2订单明细\r';
        $msg .= $products['title'] . '—' . ($products['tuan_price']/100).'元'.'*'.$order['num'].'\r';
        $msg .= '总价：' . $order['total_price']/100 . '元\r';
        if ($order['status'] == 1) {
            $status = '已经付款购买';
