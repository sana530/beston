...
class WeixintmplModel extends CommonModel{
    ...
    public function weixin_pay_balance_user($log_id){
		   ...
		   if($logs['type'] == 'tuan'){
			  ...
			  $url = $config_site_url.'cloud/detail/log_id/'.$logs['order_id'].'/';  
               ...
               $type_name = '云购';
               $url = $config_site_url.'receipt/detail/order_id/'.$logs['order_id'].'/';
           }
		   $config = D('Setting')->fetchAll();
		   $users = D('Users')->find($logs['user_id']);
    ...
    public function weixin_pay_balance_shop($log_id,$type){
		   ...
		   if($logs['type'] == 'tuan'){
			  ...
			  $order_goods = '旅行社套餐：'.$Farmpackage ['title'];
			  ...
			  $order_user_information = '购买人信息：'.$Users['nickname'].'-'.$Users ['mobile'];
		       ...
		       $sacnOrder = D('Scanorder')->find($logs['order_id']);
               $appid = D('Scanorder')->where(array('order_id'=>$logs['order_id']))->getField('appid');
               $user_id = D('Scanapikey')->where(array('appid'=>$appid))->getField('user_id');
               ...
               $shop_name = $Shop['shop_name'];
               $order_goods = '未知';
               $order_price = '订单金额：'.round($sacnOrder['need_pay']/100,2).'元';
               $order_user_information = '客户昵称：'.$Users['nickname'];
           }elseif($logs['type'] == 'receipt'){
		       $receiptOrder = D('Shopreceiptorder')->find($logs['order_id']);
               $Shop = D('Shop')->find($receiptOrder['shop_id']);
               $Users = D('Users')->find($Shop['user_id']);
               $type_name = '商家收款';
               $url = $config_site_url;
               $shop_name = $Shop['shop_name'];
               $order_goods = '未知';
               $order_price = '订单金额：'.round($receiptOrder['total_price']/100,2).'元。实际支付：'.round($receiptOrder['need_pay']/100,2).'元';
               $order_user_information = '客户昵称：'.$Users['nickname'];
           }
		   $config = D('Setting')->fetchAll();
           include_once "Baocms/Lib/Net/Wxmesg.class.php";
           $_data_order_notice = array(
			...
			if ($logs['order_id']!=0){ //多店的微信通知暂时不支持，以后再加，如果需要
...
//会员提现，审核，拒绝，通知会员自己
    ...
    //转账成功，余额变化通知双方
    public function weixin_transfer_balance_user($log_id){
        $logs = D('Usertransferlogs')->find($log_id);
        $url = 'http://' . $_SERVER['HTTP_HOST'] . '/mcenter/logs/moneylogs';
        $config = D('Setting')->fetchAll();
        $from_user = D('Users')->find($logs['user_id']);
        $to_user = D('Users')->find($logs['uid']);
        $price = round($logs['money'] / 100, 2);
        $from_user_balance = round($from_user['money'] / 100, 2);
        $to_user_balance = round($to_user['money'] / 100, 2);
        include_once "Baocms/Lib/Net/Wxmesg.class.php";
        $_data_balance_from = array(
            'url' => $url,
            'topcolor' => '#F55555',
            'first' => '您的账户余额发生变动，信息如下：',
            'remark' => '如对上述余额变动有异议，请联系'.$config['site']['sitename'].'客服人员协助处理。' . $config['site']['tel'],
            ...
            'operateInfo' => '转账',
            'price' => '-' . $price . '元',
            'balance' => $from_user_balance . '元'
        );
        $_data_balance_to = array(
            'url' => $url,
            'topcolor' => '#F55555',
            'first' => '您的账户余额发生变动，信息如下：',
            'remark' => '如对上述余额变动有异议，请联系'.$config['site']['sitename'].'客服人员协助处理。' . $config['site']['tel'],
            ...
            'operateInfo' => '转账',
            'price' => $price . '元',
            'balance' => $to_user_balance . '元'
        );
        $balance_data_from = Wxmesg::pay($_data_balance_from);
        $balance_data_to = Wxmesg::pay($_data_balance_to);
        Wxmesg::net($logs['user_id'], 'OPENTM205454780', $balance_data_from);
        Wxmesg::net($logs['uid'], 'OPENTM205454780', $balance_data_to);
        return true;
    }
