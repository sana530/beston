			...
			foreach ($coupon_download_list as $k => $val ){
			  if (!empty($val['coupon_id'])) {
					...
					if ($order['shop_id'] != $val['shop_id']) {
						...
						$downloadlist[$i]=$coupon_download_list[$k];
						$i+=1;
					}
				}
			 }
			 //取出第一条最后时间的优惠劵
			if(!empty($downloadlist[0])){
				$coupon= D('Coupon')->where(array('coupon_id'=>$downloadlist[0]['coupon_id'],'closed'=>0, 'expire_date' => array('EGT', TODAY)))->find();
				if(!empty($coupon)){
					if($order['total_price'] > $coupon['full_price']){
						$download_id = $downloadlist[0]['download_id'];
