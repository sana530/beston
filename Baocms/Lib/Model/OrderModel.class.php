...
class OrderModel extends CommonModel{
    ...
    public function overOrder($order_id){
     //后台管理员可以直接确认2的
        ...
        if ($this->save(array('status' => 8, 'order_id' => $order_id))) {
            ...
            if($shop['is_pei'] == 0){ // 商家不配送
                $config = D('Setting')->fetchAll();
                $expressprice = $order['total_express']*100 - $config['mobile']['delivery_price']*100;
            }else{ // 商家配送
                $expressprice = $order['total_express']*100;
            }
                ...
                if ($order['is_daofu'] == 0) {
                    ...
                    foreach ($goods as $val) {
                        ...
                        if ($money > 0) {
                            D('Shopmoney')->add(array(
								...
								'type' => 'goods', 
								'order_id' => $order_id, 
								'intro' => $info . ' 结算费用(平台返还商家): ' . $money/100.0
							));
                    ...
                    if ($expressprice !=0 ){
                        D('Shopmoney')->add(array(
                            ...
                            'type' => 'goods', 
                            'order_id' => $order_id, 
                            'intro' => $info . ' 运费: ' . $expressprice/100.0
                        ));
                        D('Users')->Money($shop['user_id'], $expressprice, '商户商城订单资金结算：' . $order_id);
                    }
					...
					//给推荐的驿站站长返利
                    if ($order['tui_zhanzhang'] > 0) {
					    $has_profited = D('Deliveryzhanzhangprofit')->where(array('user_id'=>$order['user_id']))->getField();
					    if (!$has_profited) {
                            $nickname = $userobj->where(array('user_id'=>$order['user_id']))->getField('nickname');
                            $zhanzhang_fan = 500;
					        D('Users')->addMoney($order['tui_zhanzhang'], $zhanzhang_fan, '驿站站长推广的用户'.$nickname.'已成功买单，已成功返利');
                            D('Deliveryzhanzhangprofit')->add(array(
                                'user_id' => $order['user_id'],
                                'delivery_uid' => $order['tui_zhanzhang'],
                                'money' => $zhanzhang_fan,
                                'order_id' => $order_id,
                                'order_type' => 2,
                                'create_time' => NOW_TIME,
  ...
  //后台退款跟商家退款更新退款库存
   public function goods_num($order_id) {
    ...
    public function goods_print($order_id,$addr_id) {
        $order_id = (int) $order_id;
        $addr_id = (int) $addr_id;
        ...
        foreach ($products as $key => $value) {
            ...
            $msg		  .= ($key+1).'. '.$product['title'].'—'.($product['mall_price']/100).'元'.'*'.$value['num']. $product['guige'] . '\r';
        }
        $msg .= '----------------------\r';
        $msg .= '应付金额：' . $order['total_price'] / 100 . '元\r';
        return $msg;//返回数组
    }
