    ...
    public function cancel($order_id){
        ...
        if(!$order_id = (int)$order_id){
            ...
            if(false !== $this->save(array('order_id'=>$order_id,'order_status'=>-1))){
                if($detail['is_fan'] == 1){
                    D('Users')->addMoney($detail['user_id'],(int)$detail['amount']*100,'旅行社订单取消,ID:'.$order_id.'，返还余额');
                }
                return true;
            }else{
                return false;
    ...
    public function complete($order_id){
        if(!$order_id = (int)$order_id){
            ...
            if(false !== $this->save(array('order_id'=>$order_id,'order_status'=>2))){
                if($detail['is_fan'] == 1){
                    D('Users')->addMoney($shop['user_id'], $detail['jiesuan_amount']*100, '旅行社订单完成,ID:'.$order_id.'，结算金额');
                }
                return true;
            }else{
                return false;
