...
function searchWordFrom() { //主要方便存入COOKIE（跟踪一个月）
    if(!empty($_GET['tuiyitui'])){//全局的推广连接可以 主要是投放广告等监控使用
        ...
        $from = 'tui';//推广
        cookie('tui_from',$keyword,6*30*86400);//存放在COOKIE 一个月
    }
...
function is0800($string) {
    if (preg_match('/^(0|(\+64(\s|-)?)){1}800(\s|-)?\d{3}(\s|-)?\d{3}$/', $string))
        return true;
    return false;
}

function isPhone($string) {
    if (preg_match('/^(0|(\+64(\s|-)?)){1}\d{1}(\s|-)?\d{3}(\s|-)?\d{4}$/', $string))
        return true;
    return false;
}
...
function isMobile($number) {
    ...
    if (preg_match('/^64[0-9]{6,9}$/', $number))
...
function isCnMobile($number) {
    if ( substr($number, 0, 1) != 1 or strlen($number) != 11) {
        return false;
    }
    return true;
}
...
function isImage($fileName) {
    ...
    $nort = in_array($ext, array('jpeg', 'jpg', 'png', 'gif', 'pdf'));
    $hext = explode('?', $ext);
    $httt = in_array($hext[0], array('jpeg', 'jpg', 'png', 'gif', 'pdf'));
    if($nort || $httt){
        return true;
    }else{
        return false;
...
function isPdf($fileName) {
    ...
    if ($ext_seg_num <= 1)
        return false;
    $ext = strtolower($ext[$ext_seg_num - 1]);
    $nort = in_array($ext, array('pdf'));
    $hext = explode('?', $ext);
    $httt = in_array($hext[0], array('pdf'));
    if($nort || $httt){
        return true;
    }else{
        return false;
...
//1st APP专用，返回完整的URL，1代表PC，2代表手机端
function config_navigation_url_app($url,$type){
    if(strstr($url,"http")){
        $url = $url;
    }elseif(strstr($url,"https")){
        $url = $url;
...
//在控制器中类似于calldata标签用法
function fetchCalldata($attr) {
...
//商城自营店选择收货时间
function collection_date () {
    $data = array();
    $today = strtotime(date('y-m-d', NOW_TIME));	//The time at 00:00:00 today
    if ((NOW_TIME - $today) < 10*3600) {
        $data[1] = date('y-m-d', NOW_TIME);
    }
    for ($i=1; $i<10; $i++) {
        $dayOfWeek = date("N", NOW_TIME+$i*24*3600);
        if ($dayOfWeek != 6 && $dayOfWeek != 7) {
            $data[$i+1] = date('Y-m-d', NOW_TIME+$i*24*3600);
        }
    }
    return $data;
}
